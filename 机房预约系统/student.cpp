...
void Student::showMyOrder() 
{
	...
	for (int i = 0; i < of.m_Size; i++) 
	{
		...
		if (this->m_Id == atoi(of.m_order[i]["stuId"].c_str())) //找到自身预约
		{
...
void Student::cancelOrder() 
{
	...
	{
		//第一个if判断了是否为本人的预约记录，
		//第二个if才判断是否能取消，所以这里显示的全是本人记录不是所有记录，不会出现记录间隔别人的所以标号对不上的情况
		//先判断是自身学号
	...
	while (true)
	{
		...
		if (select >= 0 && select <= v.size()) 
		{
			...
			{
				of.m_order[v[select - 1]]["status"] = "0";// v[select - 1]存的就是在大容器中的下标
				of.upateOrder();
